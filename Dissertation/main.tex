...
<<<<<<< HEAD
With the data points and theoretical spectra shown in Figure \ref{fig:correlation_data_final_plots}, one can find new constraints for cosmological parameters. At first, we attempted to use an MCMC method with the a Python package called Cobaya\cite{CobayaASCL, Cobaya_preprint} to determine the constraints of multiple parameters, but during our testing stages, the process showcased a very slow convergence speed even while sampling a single parameter, making this analysis unviable. Upon inspecting the partial results of this process, we have noticed that the likelihood of the parameter we tested is very wide, which strongly reduced the acceptance rate of the chain.
=======
With the data points and theoretical spectra shown in Figure \ref{fig:correlation_data_final_plots}, one can find new constraints for cosmological parameters. At first, we attempted to use an MCMC method with the a Python package Cobaya\cite{CobayaASCL, Cobaya_preprint} to determine the constraints of multiple parameters, but I was not able to implement our theoretical estimation with Cobaya efficiently, so we decided to calculate the likelihood profile for $\Omega_m$, allowing us to analyse the influence that the cross-correlation data has on constraints of this parameter.
>>>>>>> 5bef5567320e702d4ef7915fc17c14d0f46b5bbf
...
Figure \ref{fig:likelihood_prof_ctg} shows that the CMB-galaxy cross-correlation data from 2MASS does not significantly constraint the $\Omega_m$ parameter, due to the fact that the likelihoods calculated for every physically meaningful value of $\Omega_m$ is within 70\% of the maximum likelihoods calculated. 
...
Simply visualizing the gray band and comparing it with the peaks of the likelihoods indicate that bands 3 and 4 of 2MASS and the full joint-likelihood are compatible with the Planck's results within $1\sigma$, and bands 1 and 2 are also compatible. The width of the likelihoods in all bands are very similar, showing very little differences between the constraint power of each band in this analysis. 

Comparing these results with Figure \ref{fig:likelihood_prof_ctg} shows that $C^{gg}$ has a much higher constraint power over $\Omega_m$ than $C^{tg}$: while Figure \ref{fig:likelihood_prof_ctg+cgg} shows very narrow peaks in a very small region of the $\Omega_m$ space, with nearly null likelihoods outside of this small region, the results for $C^{tg}$ only show very high relative likelihood values across all physical values of $\Omega_m$, as previously discussed.
