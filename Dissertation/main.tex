...
\noindent where $\sigma_\ell$ are the errors of $C_\ell^\text{data}$. Since the only theoretical parameter being varied is $\Omega_m$, $C_\ell^\text{theo}$ is basically a function of $\Omega_m$, and so is the likelihood of each point. The combined likelihood of a single correlation spectrum is a product of the likelihoods of each multipole, so the likelihood function of the cross-correlation spectrum $C^{tg}$ is

\begin{equation}\label{likelihood_ctg}
	\mathcal{L}(\Omega_m|C^{tg}_\text{data})=\prod_{\ell=2}^{\ell_\text{max}} \mathcal{L}(C_{\ell\text{, theo}}^\text{tg}|C_{\ell\text{, data}}^\text{tg}),
\end{equation}

and the calculation of this likelihood relative to $C^{gg}$ is analogous. We have analysed the likelihoods relative to these two full correlation functions, as well as their joint likelihood:
...
We have calculated these three likelihoods for all 4 bands of the 2MASS catalog and normalized them relative to the maximum value obtained in each separate instance, the results are shown and discussed in the next section.
