...
In this part of the project, the HEALPix software was used extensively, and to validate the procedure for the search of a probability distribution for the null hypothesis, we first tested how the software would be able to retrieve the signal of a non-null cross-correlation spectrum. To do this, the full autocorrelation spectrum in \ref{fig:Ctt_ISW_comparison} calculated using CAMB was used as well as the cross-correlations \ref{fig:correlations_theoretical} as inputs for a HEALPix function that can sample values of $a_{\ell m}$ for the temperature and galaxy contrast maps that follow both the autocorrelation functions given, as well as the cross-correlation function, and a map representation can be calculated with this using another HEALPix functionality.

Figure ?? illustrates one galaxy contrast map and one CMB temperature map calculated using this procedure.

FIGURA DO MAPA HEALPIX 

It is necessary that multiple maps are synthesized this way in order to use these maps to actually retrieve the correlation spectra properly. During our tests, the autocorrelations quickly converged to their corresponding theoretical spectra, but not the cross-correlation. Figure ?? shows a comparison between the theoretical cross-correlation spectrum and the one estimated from ten thousand synthesized maps, which was the value we considered a reasonable tradeoff between quality and time efficiency for this project.

FIGURA DO CTG SINTÉTICO+TEÓRICO

The same process can be done using $C^{tg}=0$ for every multipole, giving us an estimation of the distribution of $C^{tg}(\ell)$ that can be used as a probability distribution for the null hypothesis. Figure ?? shows two histograms representing this distribution.
...
\begin{equation}\label{def:Pnull}
	P_\text{null}=\prod_{l=2}^{\ell_\text{max}} f_\ell\left[C^{tg}(\ell)\right]
\end{equation}
...
SHOW THE HEAT PLOTS, THE MINIMIZER FINAL RESULT AND DISCUSS HOW IT DOESN'T SEEM TO HAVE A RELEVANT EFFECT ON THE FINAL RESULTS
