...
\subsection{Tools Used and Convergence}

Cobaya is a Python package designed to run MCMC chains to analyse any type of data desired, but it is integrated with many sets of cosmological datasets and packages, such as Planck's data and CAMB, so it is more often used as a tool for analysis of cosmological data. Figure \ref{fig:cobaya_test} shows the results for a chain I have run to test the basic functionalities of the software, which can be compared with a similar plot in \cite{esteban_msc}, which verifies that the code was working appropriately.

\begin{figure}[!htb]
	\centering
	\includegraphics[width=\linewidth]{Imagens/compare_likelihoods.png}
	\caption{Joint posterior distributions of cosmological parameters using CMB's temperature (T) and polarization (E) data. The TTTEEE dataset combines both autocorrelation data (TT and EE) and the cross-correlation data (TE).}
	\label{fig:cobaya_test}
\end{figure}

Falar que eu usei o ctypes pra integrar o cobaya com o código em C++ e explicar o procedimento. Comentar sobre as regras de convergência do Cobaya. Discutir sobre as minhas cadeias não convergirem nem pra um parâmetro só e finalizar o capítulo passando a vez pro likelihood profiling.
