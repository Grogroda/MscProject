...
\noindent where $n_\text{dim}=N_p^2$.
...
From this sample, one has an estimation of $P(\mathbf{C}, \mathbf{s}|\mathbf{d})$, thus only needing to marginalize over $\mathbf{s}$ to obtain the posterior distribution $P(\mathbf{C}|\mathbf{d})$.

From this point, one can assume a theoretical model for $\mathbf{C}$ using a set of parameters $\theta$ and estimate the posterior distribution of these parameters $P(\theta|\mathbf{d})$ using the so called Blackwell-Rao estimator\cite{Chu_2005}.
...
With this, it is possible to arrive at the Blackwell-Rao estimator for the density $P(C_\ell|\mathbf{d})$.

\begin{equation}\label{BR_estimator}
	P(C_\ell(\theta)|\mathbf{d})\approx \frac{1}{N_G}\sum_{i=1}^{N_G}P(C_\ell|\sigma_\ell^i),
\end{equation}

\noindent where $N_G$ is the number of Gibbs samples. 
