...
For a masked map, equation \eqref{cosmic_variance} has to be modified to take into account the effective sky fraction $f_{\text{sky},\ell}$, so that in general an autocorrelation spectrum's $C_\ell^{tt}$ cosmic variance contribution can be calculated with: 

\begin{equation}\label{data_cosmic_variance}
	(\Delta C_\ell^{xx})_\text{CV}=\frac{C_\ell^{xx}}{f_{\text{sky},\ell}}\sqrt{\frac{2}{2\ell+1}}.
\end{equation}

$C_\ell^{xx}$ can be subtituted for both $C_\ell^{tt}$ and $C_\ell^{gg}$, but can also be substituted by $D_\ell^{tt}=\ell(\ell+1)C_\ell^{tt}/2\pi$ on both sides of the equation. For the cross-correlation spectrum, the cosmic variance band is slightly different:

\begin{equation}
	(\Delta C_\ell^{tg})_\text{CV}=\sqrt{\frac{2}{2\ell+1}\frac{C_\ell^{tt}C_\ell^{tg}+(C_\ell^{tg})^2}{f_{\text{sky},\ell}^2}}.
\end{equation}

The procedures discussed so far in this chapter have already been done, more specifically the Gibbs sampling has already been done in \cite{Moura-Santos_2016} and all the data has been digitally stored and provided to be used in this project. Figure \ref{fig:correlation_data_final_plots} shows a comparison between the theoretical correlation spectra and the ones calculated from the 2MASS+Q, V, W channels' data, alongside cosmic variance bands. We have taken $f_{\text{sky},\ell}=0.7$, corresponding to the percentage of the sky that was unmasked after a combination of the 2MASS and WMAP's masks.

\begin{figure}[!htb]
	\centering
	\includegraphics[width=\linewidth]{Imagens/Full_Data_Plot.png}
	\caption{Comparison of theoretical correlation spectra for each band of the 2MASS catalog and the estimated correlation values using the method presented in the current chapter. Data points with error bars are the weighted average for channels Q, V, and W, with weights given by the reciprocal of noise variances. The red band corresponds to the cosmic variance, calculated using equation \eqref{data_cosmic_variance}.}
