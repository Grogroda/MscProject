...
It was verified in \cite{Moura-Santos_2016} that fixing the monopole and dipole terms at 0 in the Gibbs chain results in anomalous states at those scales, so the WMAP9 maps were decomposed, and the results obtained for $C_0^{tt}$ and $C_1^{tt}$ were taken and fixed along the chain. 

To estimate the systematic uncertainties in the final spectrum obtained from the maps coming from the Gibbs sampling algorithm, PERGUNTAR PRO EDIVALDO COMO FOI FEITA ESSA ESTIMATIVA, NÃO ENTENDI 

This procedure has already been done in \cite{Moura-Santos_2016} and all the data has been digitally stored and provided in order to perform the analysis on cosmological constraints. Figure ?? shows a comparison between the theoretical correlation spectra and the ones calculated from the 2MASS+Q, V, W channels, alongside cosmic variance bands.

IMAGEM TIPO A FIGURA 15 DO ARTIGO

Figure ?? clearly shows that cosmic variance strongly dominates the signal at low multipoles, the region that is most influenced by the ISW effect, though it is important to highlight that the blue bands cannot be taken as the full systematic uncertainties, since only a few contributions to it have been investigated.

\chapter{Cosmological Constraints}\label{chapter:constraints}

With the data points and theoretical spectra shown in Figure ??, one can find new constraints for cosmological parameters. At first, we attempted to use an MCMC method with the a Python package called Cobaya\cite{CobayaASCL, Cobaya_preprint} to determine the cosmological constraints of multiple cosmological parameters, but I was not able to implement out theoretical estimation with Cobaya efficiently, so we decided to calculate the likelihood profile for $\Omega_m$, where $m$ indicates the matter content in the universe (see equation \eqref{eq:Omega_def} and Table \ref{tab:rhos&Omegas}), allowing us to analyse the influence that the cross-correlation data has on constraints of this parameter.
...
%1. Desprezar o efeito de reionização, já que o ISW é importante em grandes escalas e reionização é importante em escalas l >~ 200. (X)

%2. Fazer uma mudança de variáveis de tempo conforme para redshift (X)

%3. Desprezar o stress anisotrópico em tempos recentes, isso permite escrever que \Psi ~ \Phi at late times (X)

%4. Usar a equação de Poisson para relacionar o potencial gravitacional \Phi com o contraste \delta (X)

%5. Usar a definição de função de crescimento. (X)
