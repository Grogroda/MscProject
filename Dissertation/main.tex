...
Figure ?? shows the $C^{tg}+C^{gg}$ joint-likelihood calculated for each of the 4 bands of the 2MASS catalog, as well as the full 2MASS joint-likelihood of all the bands together. 

FIGURE OF CTG+CGG RESULTS

DISCUSSION OF RESULTS

The contributions of both $C^{tg}$ and $C^{gg}$ to the likelihoods shown in Figure ?? are shown in Figure ??, where the likelihoods of each single spectrum applied to their data was calculated as shown in equation \eqref{likelihood_cxy}.

FIGURES OF CTG AND CGG SEPARATELY
