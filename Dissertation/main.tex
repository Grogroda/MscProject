...
Certamente os mais importantes são os membros da minha família, da qual todos contribuíram de alguma forma, mas eu tenho a obrigação de destacar meus pais, que lutaram separadamente contra adversidades diversas para que eu chegasse onde eu cheguei, que me ensinaram o que é certo e como me dedicar, que não deixaram nada faltar para mim, e que até hoje seguem me motivando de formas diferentes, esses 2 anos de muito trabalho e cansaço me fizeram ter muito mais admiração pelo esforço de ambos. Destaco ainda meus avós e minha tia, que tiveram imensa influência em quem eu sou, e cuidaram imensamente de mim, mas também de meus dois vizinhos da frente, que podem não compartilhar do meu DNA, mas ajudaram a me criar como se eu fosse seus filhos, e eu tenho um imenso amor e gratidão por ambos.

Outra detentora de uma parcela significativa dos créditos desse trabalho pertence à minha incrível namorada, que está ao meu lado, me apoiando e tornando meus dias melhores desde os anos em que eu era um aluno de graduação, 
...
\begin{figure}[!htb]
	...
	\caption{Normalized $C^{tg}+C^{gg}$ joint-likelihoods for each band of the 2MASS catalog and full joint-likelihood of all bands.}
	\label{fig:likelihood_prof_ctg+cgg}
\end{figure}

DISCUSSION OF RESULTS

The contribution of $C^{tg}$ to the likelihoods are shown in Figure ?? are shown in Figure \ref{fig:likelihood_prof_ctg}, which was calculated using equation \eqref{likelihood_cxy}.

\begin{figure}[!htb]
	\centering
	\includegraphics[width=.7\linewidth]{Imagens/profile_ctg_Nmc2e7.png}
	\caption{Normalized $C^{tg}$ likelihood of each band of the 2MASS catalog and combination of all bands' likelihoods.}
	\label{fig:likelihood_prof_ctg}
\end{figure}

DISCUSSION OF RESULTS (CTG HAS VERY LITTLE CONSTRAINT POWER, IS THERE A RELATION BETWEEN THE ANALYSIS OF CHAPTER 3 AND THESE RESULTS? ETC)

We have also performed an analysis using artificial data following the optimized selection function discussed in Chapter \ref{chapter:select_func_optimal}. To do so (EXPLAIN THE PROCEDURE). The results of this procedure are shown in Figure \ref{fig:likelihood_band_optimal}. %o gráfico já ta exponenciado
