...
Equation \eqref{Wg} introduced the selection function $\dv{N}{z}$, which simply corresponds to the redshift distribution of galaxy number $N$ and can vary depending on the galaxy catalog used, thus having an influence on the expected signal that can be extracted from that catalog. As shown in Figure \ref{fig:correlations_theoretical}, the ISW signal is mostly present in low multipoles, where cosmic variance has a strong influence, this motivated us to study the possibility of optimizing the cross-correlation signal by determining a selection function that could theoretically maximize it, this study will be presented in this chapter.
...
To start the search for an ideal selection, a good first guess would be to find functions that favor galaxies at redshifts close to the period of transition to the current dark energy dominated universe, which has beens stated before in this text to be $z^*\approx0.63$, a value that I calculated inputing the $\Lambda$CDM values for cosmological parameter in equation\cite{dark_energy_era}

\begin{equation}\label{dark_energy_era_redshift}
	z^*=\left[-(1+3w_{DE})\frac{\Omega_{DE}}{\Omega_m}\right]^{-\frac{1}{3w_{DE}}}-1,
\end{equation}

\noindent which assumes a constant equation of state parameter $w_{DE}$ for dark energy. 
...
\begin{figure}[!htb]
	...
	\caption{Selection function calculated for various parameter values according to the parametrization \eqref{select_func_par}. Each plot is made with constant values for two parameters, corresponding to the band 2 of the 2MASS catalog (see Table \ref{tab:bands_2MASS}), and the parameter on the right is varied.}
...
These observations make it easier to change the parameters and see their influence on the cross-correlation signal and helps choose the starting region of the parameter space to explore, but to do so, the matter of how to quantify the quality of the signal has to be discussed.

\section{Maximizing the Cross-Correlation Signal}

To maximize the signal we are in search of, we compare it with the null hypothesis that there is no cross-correlation induced by the ISW effect, i.e $C^{tg}(\ell)=0$ for every multipole. To do this is, we need to estimate the probability distribution of $C^{tg}$ for each multipole $\ell$ extracted from an uncorrelated map. 

In this part of the project, the HEALPix software was used extensively, and to validate the procedure for the search of a probability distribution for the null hypothesis, we first tested how the software would be able to retrieve the signal of a non-null cross-correlation spectrum. To do this, the full autocorrelation spectrum shown in Figure \ref{fig:Ctt_ISW_comparison} calculated using CAMB was used as well as the cross-correlations shown in Figure \ref{fig:correlations_theoretical} as inputs for a HEALPix function that can sample values of $a_{\ell m}$ for the temperature and galaxy contrast maps that follow both the autocorrelation functions given, as well as the cross-correlation function, and a map representation can be calculated with this using another HEALPix functionality.

Figure ?? illustrates one galaxy contrast map and one CMB temperature map calculated using this procedure.
...
THE NUMBERS ARE UNREADABLE AND THE CTG PLOT IS ALL IN PORTUGUESE, I'LL NEED SOME TIME TO REDO THESE PLOTS SINCE I DON'T HAVE THE DATA TO PLOT THEM ANYMORE

It is necessary that multiple maps are synthesized this way in order to use them to actually retrieve the correlation spectra properly. During our tests, the autocorrelations quickly converged to their corresponding theoretical spectra, but not the cross-correlation. Figure ?? shows a comparison between the theoretical cross-correlation spectrum and the one estimated from ten thousand synthesized maps, which was the value we considered a reasonable tradeoff between quality and time efficiency for this project.

FIGURA DO CTG SINTÉTICO+TEÓRICO

In order to determine the probability distribution of the null hypothesis for each multipole, the same process can be done using $C^{tg}=0$ for every multipole, the distribution of $C^{tg}(\ell)$ can be used as a probability distribution for the null hypothesis. Figure \ref{fig:synthmaps_histograms} shows two histograms representing this distribution.
...
Upon thorough inspection of the histograms, we decided to fit each histogram using the gaussian distribution function, and using that as the distribution function of $C^{tg}$ for that multipole, each of them will be indicated by $f_\ell[C^{tg}(\ell)]$.
...
As shown in Figure \ref{fig:Triple_ColorPlots}, the region explored does not present a significant reduction in the probability of a certain parameter combination being compatible with the null hypothesis relative to the band 1 of the 2MASS catalog, showing no more than a 3\% reduction in the ratio. In spite of this, it is possible to see a region of low values for this function with where a minimizer would possibly be able to start the search for a minimum value.
...
For this combination of parameters, $P_\text{null}(z_0, \beta,\lambda)/P_\text{null}^\text{2MASS}=0.971$, still a 3\% improvement relative to band 1 of 2MASS. Figure ?? shows the selection and cross-correlation functions for this minimum, while Figure ?? shows heat plots similar to those presented in Figure \ref{fig:Triple_ColorPlots}, but these plots show the same heat maps around the minimum encountered.

FIGURAS CITADAS LOGO AGORA

According to our analysis, changing the parameters so that the selection function favors galaxies around $z^*=0.63$ does not significantly improve the signal, and in a hypothetical galaxy catalog that follows the parametrization \eqref{select_func_par}, there is not much statistical gain from using a catalog with a set parameters different from the 2MASS. Because of that, we decided to use the 2MASS catalog for the data analysis.
