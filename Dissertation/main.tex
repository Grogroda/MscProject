...
These properties are not exclusive to a particular cosmological model, instead being present in most of them. The standard model of cosmology, called $\Lambda$CDM, usually takes a few assumptions on top of the ones described previously:
...
The metric $g_{\mu\nu}$ described in equation \eqref{cap2:initial_tensors} is called the Friedmann-Lemaître-Robertson-Walker (FLRW) metric. In order to expand any equation from \eqref{field_eqs}, it is necessary to determine $R=g^{\mu\nu}R_{\mu\nu}$. By expanding the expression using the matrices defined in \eqref{cap2:initial_tensors}, the result is
...
%COMPLEMENTAR AQUI COM: VELOCIDADE DO HORIZONTE SONORO (OU SEI LÁ)
%ADICIONAR OS PLOTS QUE EU FIZ DA INFLUÊNCIA DA DARK ENERGY NO CTT
...
With the use of the geodesic equation on this new tensor and using a first order approximation, it is possible to obtain the following results for the motion equations of particles traveling through this perturbed space-time:
...
TALK ABOUT THE PARAMETRIZATION, SHOW THE PLOTS EXPLORING THIS PARAMETRIZATION AND WHICH AREAS SHOULD BE USED AS STARTING POINTS TO EXPLORE THE CROSS-CORRELATION OPTIMIZATION

\section{Maximizing the Cross-Correlation Signal}

SHOW THE PROBABILITY FUNCTION WE ARE ATTEMPTING TO OPTIMIZE, THE HEAT PLOTS EXPLORING THE REGIONS OF INTEREST AND THE FINAL RESULT OF THE MINIMIZER
...
\chapter{Data Sets and Analysis Methodology}\label{chapter:data_sets&analysis}
