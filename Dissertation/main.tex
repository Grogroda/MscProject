...
For a masked map, equation \eqref{cosmic_variance} has to be modified to take into account the effective sky fraction $f_{\text{sky},\ell}$, aside from that one can simply multiply that result by the $\ell(\ell+1)/2\pi$ term that we are using to represent the spectra (or not in the spectra that don't use this term), obtaining the following equation for the cosmic variance affecting $D_\ell^{tt}$ of the given data\cite{wmap_supplement}: 

\begin{equation}\label{data_cosmic_variance}
	\sigma_{CV}=\frac{\ell(\ell+1)}{2\pi}\frac{C_\ell^{tt}}{f_{\text{sky},\ell}}\sqrt{\frac{2}{2\ell+1}}
\end{equation}
