...
for i in range(4):
	...
	plt.figure(i)
	plt.title(r'$w_a={:.2f}$'.format(i*1/3))
	plt.xlabel(r'$\ell$')
	
	axMain=plt.subplot(111)
	axMain.errorbar(data_binned['l'], data_binned['Dl'], yerr=[data_binned['-dDl'], data_binned['+dDl']], fmt='k.', label='Dados binados')
	...
	axMain.set_xlim((simswa[i]['l'][30], 1900))
...
for i in range(3):
	...
	plt.figure(5+i)
	plt.title(r'$c_s^2={:.2f}$'.format(10**(i-2)))
	plt.xlabel(r'$\ell$')
	...
	axMain.set_xscale('linear')
	axMain.set_xlim((simscs[i]['l'][30], 1900))
	axMain.spines['left'].set_visible(False)
	axMain.yaxis.set_ticks_position('right')
	axMain.yaxis.set_visible(False)
