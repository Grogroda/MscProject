...
int main(int argc, char **argv){
  ...
  if (argc<15){
    cerr << "Usage: ./pk2ctg -pk [input power spectrum (ascii)] -wl [Omega Lambda] -wm [Omega matter] -h [H0 in km/s/Mpc] -lmax [maximum multipole] -z0 [selection function z0] -beta [selection function beta] -lbda [selection function lambda] -bg [galaxy bias] -out [output C_tg file (ascii)] -mode [0->quad./1->MC] -ncalls [Number of MC calls by Vegas] -nthreads [Number of OpenMP threads]" << endl;
  ...
  if (mode == 0)
    cerr << "Gaussian quadrature will be used as integration method." << endl;
  else
    cerr << "Monte Carlo will be used as integration method." << endl;
...
#pragma omp parallel for
  for (int l=2; l<=LBREAK; l++){
    ...
    // Starting time
    string start = getTimeStr();
    
    if (mode==0) // for l<=10, the integrands are more complicated. For l>10, Limber approximation simplifies the calculations
